...
def store_user(session: Session, profile: dict) -> None:
...
def store_repos(session: Session, repos: list[dict]) -> None:
    for repo in repos:
...
def store_repo_languages(session: Session, repository_id: int, langs: dict[str,int]) -> None:
...
def store_events(session: Session, events: list[dict]) -> None:
    for event in events:
